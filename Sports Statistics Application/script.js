let teams = [];

let team = {};
team["name"] = "Atlanta Hawks";
team["conference"] = "Eastern";
team["id"] = 1;
team["W"] = 5;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-19-22",
    opp: "Houston Rockets",
    score: [117, 107],
  },
  {
    home: true,
    date: "10-21-22",
    opp: "Orlando Magic",
    score: [108, 98],
  },
  {
    home: true,
    date: "10-23-22",
    opp: "Charlotte Hornets",
    score: [109, 126],
  },
  {
    home: false,
    date: "10-26-22",
    opp: "Detriot Pistons",
    score: [118, 113],
  },
  {
    home: false,
    date: "10-28-22",
    opp: "Detriot Pistons",
    score: [136, 112],
  },
  {
    home: false,
    date: "10-29-22",
    opp: "Milkwaukee Bucks",
    score: [115, 123],
  },
  {
    home: false,
    date: "10-31-22",
    opp: "Toronto Raptors",
    score: [109, 139],
  },
  {
    home: false,
    date: "11-02-22",
    opp: "New York Knicks",
    score: [112, 99],
  },
];
teams.push(team);

team = {};
team["name"] = "Boston Celtics";
team["conference"] = "Eastern";
team["id"] = 2;
team["W"] = 4;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-18-22",
    opp: "Philadelphia 76ers",
    score: [126, 117],
  },
  {
    home: false,
    date: "10-21-22",
    opp: "Maimi Heat",
    score: [111, 104],
  },
  {
    home: false,
    date: "10-22-22",
    opp: "Orlando Magic",
    score: [126, 120],
  },
  {
    home: false,
    date: "10-24-22",
    opp: "Chicago Bulls",
    score: [102, 120],
  },
  {
    home: true,
    date: "10-28-22",
    opp: "Cleveland Cavaliers",
    score: [123, 132],
  },
  {
    home: true,
    date: "10-30-22",
    opp: "Washington Wizards",
    score: [112, 94],
  },
  {
    home: false,
    date: "11-02-22",
    opp: "Cleveland Cavaliers",
    score: [113, 114],
  },
];
teams.push(team);

team = {};
team["name"] = "Chicago Bulls";
team["conference"] = "Eastern";
team["id"] = 3;
team["W"] = 5;
team["L"] = 4;
team["PCT"] = 0;
team["games"] = [
  {
    home: false,
    date: "10-19-22",
    opp: "Maimi Heat",
    score: [116, 108],
  },
  {
    home: false,
    date: "10-21-22",
    opp: "Washington Wizards",
    score: [100, 102],
  },
  {
    home: true,
    date: "10-22-22",
    opp: "Cleveland Cavaliers",
    score: [96, 128],
  },
  {
    home: true,
    date: "10-24-22",
    opp: "Boston Celtics",
    score: [120, 102],
  },
  {
    home: true,
    date: "10-26-22",
    opp: "Indiana Pacers",
    score: [124, 109],
  },
  {
    home: false,
    date: "10-28-22",
    opp: "San Antonio Spurs",
    score: [124, 129],
  },
  {
    home: true,
    date: "10-29-22",
    opp: "Philadelphia 76ers",
    score: [109, 114],
  },
  {
    home: false,
    date: "11-01-22",
    opp: "Brooklyn Nets",
    score: [108, 99],
  },
  {
    home: true,
    date: "11-02-22",
    opp: "Charlotte Hornets",
    score: [106, 88],
  },
];
teams.push(team);

team = {};
team["name"] = "Golden State Warriors";
team["conference"] = "Western";
team["id"] = 4;
team["W"] = 3;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-18-22",
    opp: "Los Angeles Lakers",
    score: [123, 109],
  },
  {
    home: true,
    date: "10-21-22",
    opp: "Denver Nuggets",
    score: [123, 128],
  },
  {
    home: true,
    date: "10-23-22",
    opp: "Sacramento Kings",
    score: [130, 125],
  },
  {
    home: false,
    date: "10-25-22",
    opp: "Phoenix Suns",
    score: [105, 135],
  },
  {
    home: true,
    date: "10-27-22",
    opp: "Maimi Heat",
    score: [123, 110],
  },
  {
    home: false,
    date: "10-29-22",
    opp: "Charlotte Hornets",
    score: [113, 120],
  },
  {
    home: false,
    date: "10-30-22",
    opp: "Detriot Pistons",
    score: [114, 128],
  },
  {
    home: false,
    date: "11-01-22",
    opp: "Maimi Heat",
    score: [109, 116],
  },
];
teams.push(team);

team = {};
team["name"] = "LA Clippers";
team["conference"] = "Western";
team["id"] = 5;
team["W"] = 4;
team["L"] = 4;
team["PCT"] = 0;
team["games"] = [
  {
    home: false,
    date: "10-20-22",
    opp: "Los Angeles Lakers",
    score: [103, 97],
  },
  {
    home: false,
    date: "10-22-22",
    opp: "Sacramento Kings",
    score: [111, 109],
  },
  {
    home: true,
    date: "10-23-22",
    opp: "Phoenix Suns",
    score: [95, 112],
  },
  {
    home: false,
    date: "10-25-22",
    opp: "Phoenix Suns",
    score: [94, 108],
  },
  {
    home: true,
    date: "10-23-22",
    opp: "Oklahoma City Thunder",
    score: [95, 112],
  },
  {
    home: false,
    date: "10-27-22",
    opp: "Oklahoma City Thunder",
    score: [110, 118],
  },
  {
    home: true,
    date: "10-30-22",
    opp: "New Orlean Pelicans",
    score: [91, 112],
  },
  {
    home: true,
    date: "10-31-22",
    opp: "Houston Rockets",
    score: [95, 93],
  },
];
teams.push(team);

team = {};
team["name"] = "Maimi Heat";
team["conference"] = "Eastern";
team["id"] = 6;
team["W"] = 4;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-19-22",
    opp: "Chicago Bulls",
    score: [108, 116],
  },
  {
    home: true,
    date: "10-21-22",
    opp: "Boston Celtics",
    score: [104, 111],
  },
  {
    home: true,
    date: "10-22-22",
    opp: "Toronto Raptors",
    score: [112, 109],
  },
  {
    home: true,
    date: "10-24-22",
    opp: "Toronto Raptors",
    score: [90, 98],
  },
  {
    home: false,
    date: "10-26-22",
    opp: "Portland Trail Blazers",
    score: [119, 98],
  },
  {
    home: false,
    date: "10-27-22",
    opp: "Golden State Warriors",
    score: [110, 123],
  },
  {
    home: false,
    date: "10-29-22",
    opp: "Sacramento Kings",
    score: [113, 119],
  },
  {
    home: true,
    date: "11-01-22",
    opp: "Phoenix Suns",
    score: [116, 109],
  },
  {
    home: true,
    date: "11-02-22",
    opp: "Sacramento Kings",
    score: [110, 107],
  },
];
teams.push(team);

team = {};
team["name"] = "Brooklyn Nets";
team["conference"] = "Eastern";
team["id"] = 7;
team["W"] = 2;
team["L"] = 6;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-19-22",
    opp: "New Orlean Pelicans",
    score: [108, 130],
  },
  {
    home: true,
    date: "10-21-22",
    opp: "Toronto Raptors",
    score: [109, 105],
  },
  {
    home: false,
    date: "10-24-22",
    opp: "Memphis Grizzlies",
    score: [124, 134],
  },
  {
    home: false,
    date: "10-26-22",
    opp: "Milkwaukee Bucks",
    score: [99, 110],
  },
  {
    home: true,
    date: "10-27-22",
    opp: "Dallas Mavericks",
    score: [125, 129],
  },
  {
    home: true,
    date: "10-29-22",
    opp: "Indiana Pacers",
    score: [116, 125],
  },
  {
    home: true,
    date: "10-31-22",
    opp: "Indiana Pacers",
    score: [116, 109],
  },
  {
    home: true,
    date: "11-01-22",
    opp: "Chicago Bulls",
    score: [99, 108],
  },
];
teams.push(team);

team = {};
team["name"] = "New York Knicks";
team["conference"] = "Eastern";
team["id"] = 8;
team["W"] = 3;
team["L"] = 4;
team["PCT"] = 0;
team["games"] = [
  {
    home: false,
    date: "10-19-22",
    opp: "Memphis Grizzlies",
    score: [112, 115],
  },
  {
    home: true,
    date: "10-21-22",
    opp: "Detriot Pistons",
    score: [130, 106],
  },
  {
    home: true,
    date: "10-24-22",
    opp: "Orlando Magic",
    score: [115, 102],
  },
  {
    home: true,
    date: "10-26-22",
    opp: "Charlotte Hornets",
    score: [134, 131],
  },
  {
    home: false,
    date: "10-28-22",
    opp: "Milwaukee Bucks",
    score: [108, 119],
  },
  {
    home: false,
    date: "10-30-22",
    opp: "Cleveland Cavaliers",
    score: [108, 121],
  },
  {
    home: true,
    date: "11-02-22",
    opp: "Atlanta Hawks",
    score: [99, 112],
  },
];
teams.push(team);

team = {};
team["name"] = "Toronto Raptors";
team["conference"] = "Eastern";
team["id"] = 9;
team["W"] = 5;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "10-19-22",
    opp: "Cleveland Cavaliers",
    score: [108, 105],
  },
  {
    home: false,
    date: "10-21-22",
    opp: "Brooklyn Nets",
    score: [105, 109],
  },
  {
    home: false,
    date: "10-22-22",
    opp: "Maimi Heat",
    score: [109, 112],
  },
  {
    home: false,
    date: "10-24-22",
    opp: "Maimi Heat",
    score: [98, 90],
  },
  {
    home: true,
    date: "10-26-22",
    opp: "Philadelphia 76ers",
    score: [119, 109],
  },
  {
    home: true,
    date: "10-28-22",
    opp: "Philadelphia 76ers",
    score: [90, 112],
  },
  {
    home: true,
    date: "10-31-22",
    opp: "Atlanta Hawks",
    score: [139, 109],
  },
  {
    home: false,
    date: "11-02-22",
    opp: "San Antonio Spurs",
    score: [143, 100],
  },
];
teams.push(team);

team = {};
team["name"] = "Los Angelos Lakers";
team["conference"] = "Western";
team["id"] = 10;
team["W"] = 2;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: false,
    date: "10-18-22",
    opp: "Golden State Warriors",
    score: [109, 123],
  },
  {
    home: true,
    date: "10-20-22",
    opp: "Los Angeles Clippers",
    score: [97, 103],
  },
  {
    home: true,
    date: "10-23-22",
    opp: "Portland Trail Blazers",
    score: [104, 106],
  },
  {
    home: false,
    date: "10-26-22",
    opp: "Denver Nuggets",
    score: [99, 110],
  },
  {
    home: false,
    date: "10-28-22",
    opp: "Minnesota Timberwolves",
    score: [102, 111],
  },
  {
    home: true,
    date: "10-30-22",
    opp: "Denver Nuggets",
    score: [121, 110],
  },
  {
    home: true,
    date: "11-02-22",
    opp: "New Orlean Pelicans",
    score: [120, 117],
  },
];
teams.push(team);

team = {};
team["name"] = "Charlotte Hornets";
team["conference"] = "Eastern";
team["id"] = 11;
team["W"] = 3;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: false,
    date: "10-19-22",
    opp: "San Antonio Spurs",
    score: [129, 102],
  },
];
teams.push(team);

team = {};
team["name"] = "Cleveland Cavaliers";
team["conference"] = "Eastern";
team["id"] = 12;
team["W"] = 6;
team["L"] = 1;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Detriot Pistons";
team["conference"] = "Eastern";
team["id"] = 13;
team["W"] = 2;
team["L"] = 7;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Orlando Magic";
team["conference"] = "Eastern";
team["id"] = 14;
team["W"] = 1;
team["L"] = 7;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Philadelphia 76ers";
team["conference"] = "Eastern";
team["id"] = 15;
team["W"] = 4;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Indiana Pacers";
team["conference"] = "Eastern";
team["id"] = 16;
team["W"] = 3;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Washington Wizards";
team["conference"] = "Eastern";
team["id"] = 17;
team["W"] = 4;
team["L"] = 4;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Milwaukee Bucks";
team["conference"] = "Eastern";
team["id"] = 18;
team["W"] = 7;
team["L"] = 0;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Dallas Mavericks";
team["conference"] = "Western";
team["id"] = 19;
team["W"] = 4;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Denver Nuggets";
team["conference"] = "Western";
team["id"] = 20;
team["W"] = 4;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Houston Rockets";
team["conference"] = "Western";
team["id"] = 21;
team["W"] = 1;
team["L"] = 8;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Minnesota Timberwolves";
team["conference"] = "Western";
team["id"] = 22;
team["W"] = 4;
team["L"] = 4;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Memphis Grizzlies";
team["conference"] = "Western";
team["id"] = 23;
team["W"] = 5;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Oklahoma City Thunder";
team["conference"] = "Western";
team["id"] = 24;
team["W"] = 4;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "New Orleans Pelicans";
team["conference"] = "Western";
team["id"] = 25;
team["W"] = 4;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Portland Trail Blazers";
team["conference"] = "Western";
team["id"] = 26;
team["W"] = 5;
team["L"] = 2;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Phoenix Suns";
team["conference"] = "Western";
team["id"] = 27;
team["W"] = 6;
team["L"] = 1;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "San Antonio Spurs";
team["conference"] = "Western";
team["id"] = 28;
team["W"] = 5;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Utah Jazz";
team["conference"] = "Western";
team["id"] = 29;
team["W"] = 6;
team["L"] = 3;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

team = {};
team["name"] = "Sacramento Kings";
team["conference"] = "Western";
team["id"] = 30;
team["W"] = 2;
team["L"] = 5;
team["PCT"] = 0;
team["games"] = [
  {
    home: true,
    date: "",
    opp: 0,
    score: [0, 0],
  },
];
teams.push(team);

localStorage.setItem("teams", JSON.stringify(teams));

function store() {
  let team1 = document.getElementById("team1");
  localStorage.setItem("team1", team1.value);
}
function calculateHomeWinsLosses(games) {
  let wins = 0;
  let losses = 0;

  games.forEach((game) => {
    if (game.score[0] > game.score[1] && game.home) {
      wins++;
    }
    if (game.score[0] < game.score[1] && game.home) {
      losses++;
    }
  });
  return `${wins} - ${losses}`;
}

function calculateRoadWinsLosses(games) {
  let wins = 0;
  let losses = 0;

  games.forEach((game) => {
    if (game.score[0] > game.score[1] && !game.home) {
      wins++;
    }
    if (game.score[0] < game.score[1] && !game.home) {
      losses++;
    }
  });
  return `${wins} - ${losses}`;
}

function createTable(teams) {
  let EasternTableBody = document.querySelector("#Estandings tbody");
  EasternTableBody.replaceChildren();
  let WesternTableBody = document.querySelector("#Wstandings tbody");
  WesternTableBody.replaceChildren();

  teams.forEach((team) => {
    const row = document.createElement("tr");
    let coll = document.createElement("td");
    coll.textContent = team.name;
    row.appendChild(coll);
    coll = document.createElement("td");
    coll.textContent = team.W;
    row.appendChild(coll);
    coll = document.createElement("td");
    coll.textContent = team.L;
    row.appendChild(coll);
    coll = document.createElement("td");
    coll.textContent = ((team.W / (team.W + team.L)) * 100).toFixed(1);
    row.appendChild(coll);
    coll = document.createElement("td");
    coll.textContent = calculateHomeWinsLosses(team.games);
    row.appendChild(coll);
    coll = document.createElement("td");
    coll.textContent = calculateRoadWinsLosses(team.games);
    row.appendChild(coll);
    if (team.conference === "Western") WesternTableBody.appendChild(row);
    else EasternTableBody.appendChild(row);
  });
}

// Sort order: "", "ASC", "DESC"
const standingSortOrder = {
  name: "",
  W: "",
  L: "",
  PCT: "",
};

function sort(field) {
  if (standingSortOrder[field] === "") {
    standingSortOrder[field] = "ASC";
  } else if (standingSortOrder[field] === "ASC") {
    standingSortOrder[field] = "DESC";
  } else if (standingSortOrder[field] === "DESC") {
    standingSortOrder[field] = "ASC";
  }
  if (field === "name") {
    teams = teams.sort((teamA, teamB) => {
      if (standingSortOrder[field] === "ASC") {
        return teamA.name < teamB.name ? -1 : 1;
      } else if (standingSortOrder[field] === "DESC") {
        return teamA.name > teamB.name ? -1 : 1;
      } else return 0;
    });
  } else if (field === "W") {
    teams = teams.sort((teamA, teamB) => {
      if (standingSortOrder[field] === "ASC") {
        return teamA.W - teamB.W;
      } else if (standingSortOrder[field] === "DESC") {
        return teamB.W - teamA.W;
      } else return 0;
    });
  } else if (field === "L") {
    teams = teams.sort((teamA, teamB) => {
      if (standingSortOrder[field] === "ASC") {
        return teamA.L - teamB.L;
      } else if (standingSortOrder[field] === "DESC") {
        return teamB.L - teamA.L;
      } else return 0;
    });
  } else if (field === "PCT") {
    teams = teams.sort((teamA, teamB) => {
      return teamA.PCT - teamB.PCT;
    });
  }
  return teams;
}

createTable(sort("PCT"));
